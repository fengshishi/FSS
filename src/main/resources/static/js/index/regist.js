...
$(function(){
	...
	$("#dt").blur(function(){
		 if($("#dt").val() == ""){
			$("#tip5").html("时间不为空 ").css("color","red");
			dt= false;
		}
		else{
			$("#tip5").html("输入格式正确").css("color","green");
			dt== true
		}
	});
	
	$("#register").click(function(){
		if(user_Boolean && password_Boolean && email_Boolean && phone_Boolean && dt== true){
			...
			$.ajax({
				type:"POST",
				url:$("#PageContext").val()+"/user/regist",
				dataType:"text",
				data:user,
				success:function(data){
					alert(data+"注册成功"); 
					alert($("#PageContext").val()+"/user/tologin");
					window.location.href = $("#PageContext").val()+"/user/tologin";
				},
				error:function(data){
					alert(data+"注册失败"); 
				}
