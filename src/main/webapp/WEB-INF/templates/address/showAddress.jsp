								...
								<table class="address-info-table bd-1-e8" border="0" cellpadding="0" cellspacing="0">
									<tbody>
										<tr class="info-title ta-ct">
											<th class="item bb-1-e8" width="20%">收货人姓名</th>
											<th class="item bb-1-e8" width="15%">联系电话</th>
											<th class="item bb-1-e8" width="45%">收货地址</th>
											<!-- <th class="item bb-1-e8" width="15%">收货地址(市)</th>
											<th class="item bb-1-e8" width="15%">收货地址(区)</th>
											<th class="item bb-1-e8" width="15%">详细地址</th> -->
											<th class="item bb-1-e8" width="20%">操作</th>
										</tr>

										<c:forEach items="${requestScope.addressList}" var="address" varStatus="statu">
											<tr class="info-text ta-ct">
												<td class="item">${address.addressName }</td>
												<td class="item">${address.addressPhone }</td>
												<td class="item">
													${address.addressProvice }
													${address.addressCity }
													${address.addressXianqu }
													${address.addressDetail }
												</td>
												<%-- <td class="item">${address.addressCity }</td>
												<td class="item">${address.addressXianqu }</td>
												<td class="item">${address.addressDetail }</td> --%>
												<td class="item">
												<a class="del-address cl-bl-l" href="${pageContext.request.contextPath }/address/upfindById?addressId=${address.addressId}">修改</a>&nbsp;&nbsp;
												<a class="del-address cl-bl-l" href="${pageContext.request.contextPath }/address/delete?addressId=${address.addressId}">删除</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
