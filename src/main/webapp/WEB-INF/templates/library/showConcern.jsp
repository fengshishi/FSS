...
<body>
	...
	<div class="content">
		<table class="table  table-striped table-bordered table-hover">
				<tr>
					<button type="button" class="btn btn-info" style="width: 150px">出版社资料</button>
				</tr>
				<tr >
				   <th width="20%">
					<p class="alert alert-info">出版社名</p>
					<p class="alert alert-info">出版数量</p>
					</th>
				
                 <th width="40%">
				<c:choose>
					<c:when test="${not empty requestScope.ylbook.ylbookConcernList }">
						<c:forEach items="${requestScope.ylbook.ylbookConcernList }"
							var="concern" varStatus="statu">
								<p class="alert alert-success">${concern.ylconcernName }</p>
								<p class="alert alert-success">${concern.ylconcernNum }</p>
						</c:forEach>
					</c:when>
					<c:otherwise>
    		暂时没有查到出版社信息！
		...
		<table>
			<tr>
				<th>
					<button type="button" class="btn btn-warning" style="width: 150px">图书集结</button>
					<h2 style="color: black; font-family:"宋体";font-weight:bolder;">拾贝网的图书日历</h2>

					<p style="font-size: 16px;">这里是图书信息的集结地。
						拾贝网的图书日历资料全部聚集。想要了解拾贝网所有的图书， 请点击右下角。</p>

					<div class="text-right">
						<a
							href="${pageContext.request.contextPath }/date/dateList">
